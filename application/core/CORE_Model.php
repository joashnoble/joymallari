...
//jbpv check if used
    function checkifused($filter) {
        $query = $this->db->query('SELECT * FROM '.$this->table.'
             INNER JOIN '.$this->tabletocheck.' ON '.$this->table.'.'.$this->pk_id.'='.$this->tabletocheck.'.'.$this->pk_id.'
             WHERE '.$this->tabletocheck.'.is_deleted=0 AND '.$this->table.'.'.$this->pk_id.'='.$filter.' LIMIT 1
            ');
                            $query->result();
